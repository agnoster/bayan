function AudioSynthView() {
  ...
  __audioSynth.loadSoundProfile({
      // name it
      name: 'bayan',
      // WIP: return the length of time, in seconds, the attack lasts
      attack: function(samplerate, frequency, volume) { return 0.0 },
      // WIP: return a number representing the rate of signal decay.
      // larger = faster decay
      dampen: function(sampleRate, frequency, volume) { return 0; },
      ...
      wave: function(i, sampleRate, frequency, volume) {
        ...
        return this.modulate[1](
          ...
          base(i, sampleRate, frequency, 0) +
            ...
            0.20*base(i, sampleRate, frequency, 0.125)
        );
      }
  });
  ...
  var note_manifold = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"];
  var key_manifold = "qazwsxedcrfvtgbyhnujmik,ol.p;/['⇧]⏎".toUpperCase().split("");
  var extra_keys =   "  2  3  4  5  6  7  8  9  0  -  =";
  var keyboard = {};
  var char_to_keycode = {
    "\\": 220,
    "[": 219,
  ...
  function keycode(char) {
    return char_to_keycode[char] || char.charCodeAt(0);
  }

  key_manifold.forEach(function(char, idx) {
    var note = note_manifold[idx % note_manifold.length];
    var register = Math.floor(idx / note_manifold.length) - 1;
    var note_code = [note, register].join(",");
    keyboard[keycode(char)] = note_code;
    if (idx < extra_keys.length && extra_keys[idx] !== " ") {
      keyboard[keycode(extra_keys[idx])] = note_code;
    }
  })
	...
	var fnPlayNote = function(note, octave) {
		...
		src = __audioSynth.generate("bayan", note, octave, 15);
	...
	var fnPlayKeyboard = function(e) {
      ...
      keysPressed.push(e.keyCode);
			...
			var container = fnPlayNote(note, __octave + octaveModifier);
      notesPlaying.push(container);
